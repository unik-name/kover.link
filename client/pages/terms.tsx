...
const TermsPage = () => (
  <AppWrapper>
    ...
    <Col width={600} maxWidth="97%" alignItems="flex-start">
      ...
      <p>
        In no event shall {process.env.NEXT_PUBLIC_SITE_NAME} or its suppliers be liable for
        any damages (including, without limitation, damages for loss of data or
        profit, or due to business interruption) arising out of the use or
        inability to use the materials on {process.env.NEXT_PUBLIC_DEFAULT_DOMAIN} website,
        even if {process.env.NEXT_PUBLIC_SITE_NAME} or a {process.env.NEXT_PUBLIC_SITE_NAME} authorized
      ...
      <p>
        The materials appearing on {process.env.NEXT_PUBLIC_SITE_NAME} website could include
        technical, typographical, or photographic errors.{" "}
        {process.env.NEXT_PUBLIC_SITE_NAME} does not warrant that any of the materials on
        its website are accurate, complete or current. {process.env.NEXT_PUBLIC_SITE_NAME}{" "}
        may make changes to the materials contained on its website at any time
        without notice. However {process.env.NEXT_PUBLIC_SITE_NAME} does not make any
        commitment to update the materials.
      </p>
      <p>
        {process.env.NEXT_PUBLIC_SITE_NAME} has not reviewed all of the sites linked to its
        ...
        {process.env.NEXT_PUBLIC_SITE_NAME} of the site. Use of any such linked website is
        at the {"user's"} own risk.
      </p>
      <p>
        {process.env.NEXT_PUBLIC_SITE_NAME} may revise these terms of service for its
